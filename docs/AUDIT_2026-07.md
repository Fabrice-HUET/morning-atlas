# Audit Morning Atlas — audit_fable.md

> Audit réalisé le 2026-07-02 sur l'état réel du repo (branche `main`, commit `783a7f9` + modifications non committées).
> Vérifications exécutées : `pnpm lint` ✅, `pnpm typecheck` ✅, `pnpm build` ✅ (652 pages générées), script d'intégrité des données exécuté via tsx.
> Tout ce qui suit a été constaté dans les fichiers ; quand quelque chose n'a pas pu être vérifié (ex. comportement en production Vercel, Search Console), c'est signalé explicitement.

---

## SOMMAIRE — État général

**Le socle technique est solide et sain.** Lint, typecheck et build passent. L'architecture App Router est propre, le TypeScript est strict et bien typé, le socle SEO P0 (sitemap, robots, canonicals, OG, JSON-LD) est réellement en place, les 53 images WebP sont alignées sur les slugs, et l'intégrité référentielle des données est parfaite (0 slug dupliqué, 0 référence cassée — vérifié par script).

**Le blocant n'est pas technique, il est éditorial.** Les 53 fiches pays et les 53 recettes sont à 100 % en `needsReview: true`, et la quasi-totalité des textes de données est écrite **sans accents français** (2 descriptions sur 53 contiennent un caractère accentué — y compris le H1 de la page d'accueil : « Decouvrir le monde a travers les petits-dejeuners »). En l'état, le site n'est pas publiable pour une marque éditoriale française qui promet la précision.

**Trois risques structurels** : (1) une inflation taxonomique massive — 410 ingrédients, 547 tags, 123 catégories pour seulement 53 pays/53 recettes, dont 410 pages ingrédients générées et indexables ; (2) un fallback d'image `picsum.photos` qui peut afficher une photo aléatoire à la place d'un petit-déjeuner (contradiction directe avec la règle de précision culturelle) ; (3) du JSON-LD Breadcrumb qui pointe vers `/categories` et `/ingredients`, deux pages index qui **n'existent pas** (404).

### Top 5 des priorités

1. **DATA-01** — Restaurer les accents français dans toutes les données éditoriales (bloquant, le site n'est pas montrable en l'état).
2. **EDIT-01** — Relire un premier lot de fiches pilotes et sortir du 100 % `needsReview` (c'est l'objet même de la phase de validation 30 jours).
3. **SEO-01** — Corriger les breadcrumbs JSON-LD qui référencent des pages 404 (`/categories`, `/ingredients`).
4. **IMG-01** — Supprimer le fallback `picsum.photos` (photo aléatoire = violation de la précision culturelle + dépendance externe).
5. **UI-01** — Ne plus afficher « A verifier » comme badge public sur les cartes et fiches (signal de non-qualité pour les visiteurs et pour Google).
...
**État constaté**
- Structure conforme au plan : `app/`, `components/` (cards, content, creator, images, layout, sections, ui), `data/`, `types/`, `lib/`, `public/`.
- TypeScript strict activé, usage de `satisfies Recipe[]`, types centralisés dans `types/`, alias `@/*`. Aucune erreur `tsc --noEmit`.
- Next.js **16.2.9** (App Router), React 19.2.4, Tailwind CSS 4 (`@theme`), pnpm. Attention : `AGENTS.md` rappelle que cette version de Next diffère des conventions connues — la doc embarquée existe bien dans `node_modules/next/dist/docs/`.
- Les recettes sont bien découpées en un fichier par recette (`data/recipes/<région>/<pays>/<slug>.ts`) agrégées dans `data/recipes.ts`.
- `lib/content-validation.ts` fournit un rapport d'intégrité complet (doublons + références manquantes)… mais n'est branché nulle part (ni script pnpm, ni CI, ni build).
- `data/countries.ts` utilise une factory `makeCountry()` qui injecte des textes par défaut génériques (heroTitle templaté, culturalNote templatée, funFact identique) — pratique pour seeder, mais risque de contenu dupliqué/générique si les fiches ne sont pas individualisées.

**Problèmes**
- Fichier parasite **`main` (vide) à la racine du repo** — artefact accidentel (probablement un `git pull main` raté). *(mineur)*
- **`app/docs/*.md`** : 18 fichiers Markdown de suivi éditorial placés **dans `app/`**. Ils ne créent pas de routes (pas de `page.tsx`), mais c'est le mauvais dossier — le suivi vit déjà dans `docs/`. *(mineur)*
- `.idea/` (config JetBrains, y compris `workspace.xml`) et `tsconfig.tsbuildinfo` sont versionnés. *(cosmétique)*
- **Travail non committé sur `main`** : page `/creator` complète, `data/creator.ts`, `components/creator/CreatorScene.tsx`, dépendance `three` ajoutée à `package.json`, lien footer. Pas de branche, pas de commit — risque de perte et de mélange avec d'autres travaux. *(important)*
- Double convention sémantique sur `Recipe` : `tags: string[]` (slugs, champ requis) **et** `tagSlugs?: string[]` (autres slugs). Exemple `tartines.ts` : `tags: ['sweet-morning', …]` et `tagSlugs: ['france', 'tartine', …]`. La page recette affiche `recipe.tags` via `TagBadge` ; `tagSlugs` ne sert que de maillage de données. Sémantique non documentée, source de confusion. *(important)*
- Champs redondants non arbitrés : `estimatedTimeMinutes` vs `prepTimeMinutes + cookTimeMinutes`, `difficulty` vs `recreateAtHomeLevel`, `culturalNote` vs `culturalContext`, `associatedDrink` vs `commonDrinks`. *(mineur)*
- Aucun test (aucun framework installé). Acceptable en phase de test, mais la validation d'intégrité mérite au minimum un script. *(mineur)*
- `docs/cv.html` non committé traîne dans le repo (CV personnel dans un repo de marque grand public — à décider). *(mineur)*

**Gravité globale : important** (rien ne casse, mais l'hygiène git et la double sémantique tags créeront des erreurs).

### 2. Routing & pages

**État constaté (vérifié au build : 652 pages)**
| Route | État |
| --- | --- |
| `/` | ✅ complète (hero, promesse, pays/recettes/catégories vedettes, roadmap, bandeau construction) |
| `/about` | ✅ existe, mais courte (2 blocs) |
| `/countries` + `/countries/[slug]` (53) | ✅ complètes |
| `/recipes` + `/recipes/[slug]` (53) | ✅ complètes |
| `/categories/[slug]` (123) | ✅ existe — **mais pas de page index `/categories`** |
| `/ingredients/[slug]` (410) | ✅ existe — **mais pas de page index `/ingredients`** |
| `/guides` | ✅ existe (3 teasers) — **pas de `/guides/[slug]`**, les guides ne sont pas cliquables |
| `/creator` | ✅ existe (non committée). Titre « À propos du créateur » ✅, lien footer uniquement ✅, absente de la navbar ✅ |
| `not-found.tsx` | ❌ absent (404 par défaut de Next, hors identité visuelle) |
| `/tags/[slug]` | ❌ n'existe pas alors que 547 tags sont maintenus dans les données |

**Problèmes**
- **Les breadcrumbs JSON-LD des pages catégories et ingrédients référencent `/categories` et `/ingredients`, qui renvoient 404.** Données structurées incohérentes soumises à Google. *(important, quasi bloquant SEO)*
- Guides annoncés dans la navbar et le footer mais sans pages de destination individuelles — la page `/guides` est un cul-de-sac éditorial assumé (« à venir »), acceptable en phase test. *(mineur)*
- 547 tags sans aucune route : les tags ne servent qu'à afficher des labels de badge. Dette de données pure. *(important, croisé avec DATA)*
- Pas de page 404 personnalisée. *(mineur)*

**Gravité globale : important.**

### 3. Données

**État constaté (compté par script, pas estimé)**
- 53 pays, 53 recettes, 123 catégories, **410 ingrédients**, **547 tags**, 3 guides.
- `needsReview: true` : **53/53 pays, 53/53 recettes (100 %)**.
- `featured: true` : **52 recettes sur 53** — le flag ne discrimine plus rien (la home coupe à 4 par `slice`).
- Intégrité : 0 slug dupliqué, 0 référence cassée (rapport `getContentIntegrityReport()` vide). Très bon point.
- Orphelins : 8 ingrédients jamais référencés, 1 tag jamais référencé, 33 pages ingrédients avec ≤ 1 contenu lié.
- Qualité culturelle du fond : réellement soignée. Formulations prudentes (« souvent associé à », « sans résumer toutes les habitudes »), champs `variants`, `reviewNotes`, `sources` remplis sur les 53 fiches avec éditeurs identifiés (Le Monde, Taste France…). La règle de non-généralisation est respectée dans la structure même des données.

**Problèmes**
- **Textes systématiquement non accentués** dans `data/countries.ts`, les fichiers recettes, `HeroSection`, `Footer`, `EditorialPromise`, `FutureRoadmap`, et les pages pays/recette (« Difficulte maison », « Recettes associees », « Ingredients lies », « Repere pratique », « Etapes »…). Vérifié : 2/53 `shortDescription` pays contiennent un accent. C'est LE chantier bloquant. *(bloquant)*
- Inflation taxonomique : 410 ingrédients + 547 tags + 123 catégories pour 53 contenus réels. L'audit interne (`TAXONOMY_SEO_AUDIT.md`) le documente déjà : 302 ingrédients « à enrichir », 33 « très faibles ». Le stock de taxonomies dépasse largement ce que la phase de test peut valider. *(important)*
- `featured` non significatif (52/53). *(mineur)*
- Enums affichés bruts en anglais dans l'UI : `difficulty: 'easy'`, `Type : grain`, styles `savory`… sur des pages entièrement francophones. *(important)*
- Double champ tags (cf. axe 1). *(important)*
- La validation d'intégrité n'est exécutée par personne automatiquement. *(important)*

**Gravité globale : bloquant** (accents + relecture), le reste important.

### 4. SEO

**État constaté**
- Vrai socle P0 en place et vérifié dans le code : `metadataBase`, canonicals via `alternates.canonical`, OG + Twitter cards par page, `generateMetadata` sur les 4 familles dynamiques, `generateStaticParams` partout, `app/robots.ts`, `app/sitemap.ts` (33 ingrédients faibles exclus), JSON-LD `WebSite`, `Organization`, `WebPage`, `BreadcrumbList`, `ItemList`, `Recipe`, sérialisé avec échappement `<`.
- Tout est SSG : excellent pour le crawl.

**Problèmes**
- **Le H1 de la home — la requête de marque principale — est non accentué** : « Decouvrir le monde a travers les petits-dejeuners ». Idem pour les `seoTitle` par défaut de la factory (« petit-dejeuner en Egypte »). Google traite ça comme du texte mal orthographié. *(bloquant)*
- **Breadcrumbs JSON-LD vers des URLs 404** (`/categories`, `/ingredients`) — cf. axe 2. *(important)*
- **410 pages ingrédients générées et indexables** : l'exclusion du sitemap ne retire pas de l'index (les pages restent maillées et crawlables, aucun `noindex`). Le risque d'inflation de pages pauvres est le plus gros risque SEO du site ; l'audit interne le sait mais la mesure prise (sitemap seulement) est cosmétique. *(important)*
- Pas d'image Open Graph par défaut : la home, `/about`, `/guides`, les catégories et ingrédients partent en `twitter: summary` sans visuel. *(important)*
- JSON-LD `Recipe` minimal alors que les données existent : pas de `prepTime`/`cookTime` (ISO 8601), pas de `recipeYield`, pas de `recipeCategory`/`keywords`. C'est ce qui déclenche les rich results recettes. *(important)*
- Liste d'exclusion sitemap **codée en dur** (33 slugs) : dérive garantie quand les données bougent — le doublon `fresh-fruit` a déjà montré le problème. *(mineur)*
- Pas de `lastModified` dans le sitemap ; pas de `title.template` dans le layout (suffixe « — Morning Atlas » répété à la main, la page recette l'ajoute, la page catégorie aussi, mais rien ne garantit la cohérence). *(mineur)*
- Non vérifiable ici : comportement réel Search Console, Rich Results Test (nécessite le site déployé). *(signalé, non constaté)*

**Gravité globale : important**, avec un composant bloquant (accents dans H1/seoTitle).

### 5. Images / assets

**État constaté**
- Convention propre et **100 % couverte** : `public/images/breakfasts/{slug-recette}.webp`, 53/53 présents, vérifié par script ; chaque pays a au moins une recette donc une image héro valide.
- `BreakfastImage` : composant client avec `next/image`, `fill`, `sizes` corrects, fallback en cas d'erreur.
- Icônes complètes (favicon, apple-icon, série icon-16 → 1024).

**Problèmes**
- **Le fallback charge `https://picsum.photos/seed/{slug}`** : une photo *aléatoire* (paysage, immeuble, n'importe quoi) serait affichée comme « petit-déjeuner de X ». Contradiction frontale avec la précision culturelle et l'identité visuelle « pas de rendu générique » ; dépendance externe + `remotePatterns` dans `next.config.ts` uniquement pour ça. *(important — le seul garde-fou est que les 53 images existent aujourd'hui)*
- Assets du template create-next-app encore présents : `next.svg`, `vercel.svg`, `file.svg`, `globe.svg`, `window.svg`. *(cosmétique)*
- Icônes dupliquées entre `app/` (icon.png, apple-icon.png, favicon.ico) et `public/` (les mêmes + favicon.ico). Next sert celles d'`app/` ; celles de `public/` sont partiellement redondantes. *(cosmétique)*
- Pas d'image sociale par défaut (cf. SEO). Pas d'illustrations pour catégories/ingrédients/guides — acceptable en phase test, à ne pas produire en masse maintenant. *(mineur)*

**Gravité globale : important** (à cause du fallback picsum uniquement).

### 6. Identité visuelle & design system

**État constaté**
- Palette Mocha Mousse bien tokenisée dans `@theme` Tailwind 4 : `mocha, cream, oat, toast, espresso, honey, sage, berry, paper, ink` — cohérente et utilisée partout via classes (`bg-cream`, `text-espresso`…).
- Langage visuel homogène : cartes `bg-paper` bordées `oat`, pills arrondies, eyebrows en uppercase tracking large. L'esprit « carnet de voyage flat » est là.

**Problèmes**
- **Typographie : Arial/Helvetica en dur dans `globals.css`.** Aucune fonte de marque, aucun usage de `next/font` (le README parle de Geist, mais rien n'est branché). Pour un média éditorial visuel, c'est l'écart le plus visible avec l'ambition. *(important)*
- **Tokens dupliqués** : la palette existe deux fois (`@theme --color-*` ET `:root --ma-*`) + valeurs hex recopiées en dur dans `CreatorScene.tsx` (0xa47864…) et dans des gradients arbitraires (`bg-[radial-gradient(...#fff6ea...)]`). Risque de dérive si la palette évolue. *(mineur)*
- Incohérences de radius : `rounded-lg` sur les cartes du site vs `rounded-2xl`/`rounded-3xl` sur la page creator et le bandeau construction. *(cosmétique)*
- Badge « A verifier »/« Publie » sur les `CountryCard` et fiches : vocabulaire d'outil interne exposé aux visiteurs, hors ton de marque. *(important — croisé UI-01)*
...
**État constaté**
- Tout est statique (SSG), images WebP locales via `next/image` avec `sizes` soignés et `priority` sur les héros : les Core Web Vitals de base devraient être bons. (Non mesuré ici — pas de Lighthouse exécuté.)
- A11y réelle déjà travaillée : `aria-expanded`/`aria-controls` sur le burger, fermeture Échap, `tabIndex` conditionnels dans le panneau mobile, `aria-label` sur les emojis drapeaux, HTML sémantique correct (`main`, `article`, `section`, `dl`, un seul H1 par page via `SectionHeading level`).

**Problèmes**
- Menu mobile : pas de focus trap (le focus peut sortir du panneau ouvert vers la page en arrière-plan) ; pas de retour de focus sur le bouton à la fermeture. *(important)*
- Pas de skip-link « Aller au contenu ». *(mineur)*
- `scroll-behavior: smooth` global et animation permanente `requestAnimationFrame` dans `CreatorScene` sans respect de `prefers-reduced-motion`. *(mineur)*
- `three` (~700 kB de JS client avant tree-shaking) chargé pour une scène décorative sur `/creator` — import dynamique certes, mais c'est le plus gros coût perf du site pour la page la moins stratégique. *(important, croisé PAGE-01)*
- Contrastes non audités (le `text-cream/60` sur `bg-espresso` du footer et `text-espresso/75` sur `cream` méritent vérification WCAG AA). *(mineur, à mesurer)*

**Gravité globale : mineur à important** (rien de cassé, deux vrais chantiers : focus trap et three.js).

### 8. Éditorial & contenu

**État constaté**
- Le ton est juste : chaleureux, précis sans être académique, et la prudence culturelle est **structurellement** appliquée (champs `variants`, `culturalContext`, formulations non généralisantes, sources visibles en bas de page, `reviewNotes` internes pertinentes — ex. tartines : « Ne pas présenter la tartine comme unique petit-déjeuner français »).
- Le bandeau « Site en construction » assume honnêtement la phase de test. Bien.
- 53 fiches pays + 53 recettes + sources : le volume pilote est là.

**Problèmes**
- **Rien n'est validé** : 100 % `needsReview`. La phase de test de 30 jours ne peut rien conclure tant qu'aucun lot n'est relu et assumé. *(bloquant)*
- Accents manquants = le ton perçu s'effondre quelle que soit la qualité du fond. *(bloquant, croisé DATA-01)*
- Mots anglais résiduels côté UI : « Breakfast world guide » sous le logo, enums bruts (`easy`, `grain`, `savory`). *(important)*
- Guides : 3 teasers sans contenu — annoncés en navigation principale. Soit assumer « à venir » plus clairement, soit écrire 1 guide pilote. *(mineur en phase test)*
- `/about` très courte pour un site qui revendique une méthode éditoriale : la méthode (sources, relecture, limites) n'est pas expliquée publiquement. *(mineur)*

**Gravité globale : bloquant** (relecture + accents), le fond est bon.

### 9. Tooling & qualité

**État constaté**
- `pnpm lint` ✅ (ESLint 9 flat config + next/core-web-vitals + next/typescript), `pnpm typecheck` ✅, `pnpm build` ✅ — vérifiés pendant cet audit.
- Scripts pnpm minimalistes mais suffisants. `pnpm-workspace.yaml` avec `ignoredBuiltDependencies`.

**Problèmes**
- **Aucune CI** (pas de `.github/`) : rien n'empêche de pousser un commit qui casse le build — Vercel le détectera au déploiement, trop tard. *(important)*
- Environnement Node non contraint : le shell système est en Node 18 (Next 16 exige ≥ 20.9 — le build a nécessité le Node 24 de nvm) ; pas de champ `engines` dans `package.json`, pas de `.nvmrc`. Piège garanti pour tout contributeur ou outil. *(important)*
- `lib/content-validation.ts` jamais exécuté automatiquement. *(important)*
- Pas de Prettier configuré (le style est cohérent, probablement à la main). *(cosmétique)*
- Aucun test — acceptable en phase test si la validation de données tourne en CI. *(mineur)*

**Gravité globale : important.**

### 10. Risques & incohérences transverses

1. **Publier en l'état ferait exister durablement dans l'index Google un site français sans accents avec 410 pages fines** — le coût de rattrapage SEO serait supérieur au coût de correction actuel. *(bloquant)*
2. **Le fallback picsum peut afficher une image sans rapport présentée comme culturellement située** — atteinte directe à la promesse. *(important)*
3. **Page `/creator`** : conforme aux consignes sur les points vérifiables (titre « À propos du créateur », lien footer uniquement, hors navbar, palette Mocha respectée, scène 3D construite en primitives three.js originales — **aucun asset copié constaté**). Deux réserves : (a) le *concept* « scène 3D de bureau de dev » reste très proche de l'univers craftzdog — la version fallback illustrée est plus originale et plus dans l'identité ; (b) +1 dépendance lourde (`three`) pendant une phase où la consigne est de ne pas surconstruire. À trancher. Incohérence factuelle : email `gmail` dans `data/creator.ts`, CV pointé vers `/files/fabrice-huet-cv.pdf` inexistant (un `docs/cv.html` non committé traîne). *(important)*
4. **Dérive de gouvernance des données** : liste d'exclusion sitemap en dur, `featured` non significatif, double champ tags, factory qui injecte du texte générique — quatre mécanismes qui divergeront silencieusement. *(important)*
5. **Hygiène git** : travail significatif non committé sur `main`, fichier `main` parasite, `.idea/` versionné. *(mineur mais à régler tout de suite, c'est 10 minutes)*
6. Les 18 fichiers de suivi dans `app/docs/` + 8 docs dans `docs/` + CHANGELOG se recoupent beaucoup : le suivi projet coûte cher à maintenir à jour. *(cosmétique)*

---

## LIVRABLE 2 — BACKLOG D'UPGRADE

Trié par priorité (P0 → P3). IDs stables par catégorie. Les tâches marquées **[sur-ingénierie en phase test]** sont volontairement classées P3.
...
---
### [PAGE-01] Finaliser la page /creator (« À propos du créateur »)
...
| Priorité | Tâches |
| --- | --- |
| **P0** | DATA-01, EDIT-01, SEO-01, IMG-01, UI-01, CODE-01 |
| **P1** | PAGE-01 (creator), SEO-02→06, DATA-02→06, PAGE-02→04, DESIGN-01/02, A11Y-01, TOOL-01/02, IMG-02, EDIT-02 |
| **P2** | DATA-07, CODE-02/03, EDIT-03/04, SEO-07, PERF-01, A11Y-02, DESIGN-03, DOCS-01 |
| **P3** (sur-ingénierie en phase test, assumée) | PAGE-05, TOOL-03, FEAT-01→04, SEO-08 |

**Ordre d'exécution recommandé pour la phase de test 30 jours :** CODE-01 (10 min) → DATA-01 → UI-01 + IMG-01 + SEO-01 (une journée) → EDIT-02 puis EDIT-01 (le cœur de la validation) → PAGE-01 → le reste du P1 au fil de l'eau. Tout le P3 attend la fin de la phase de test.
